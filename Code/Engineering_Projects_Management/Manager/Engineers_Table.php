...
$sql = "SELECT u.userid, u.userName, GROUP_CONCAT(p.ProjectID) AS projects, u.email, u.phone, u.joinDate
        FROM users AS u 
        JOIN projects AS p ON u.userId = p.SupervisingEngineerID
        WHERE u.userType = 3";

// إضافة الفلتر إذا كان موجودًا
if (!empty($filter_year)) {
    $sql .= " AND YEAR(u.joinDate) = '$filter_year'";
}

$sql .= " GROUP BY u.userid, u.userName, u.email, u.phone, u.joinDate
...
if ($result === false) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>" . $row["userid"] . "</td>"; // استخدام "userid" بدلاً من "userId"
